...
public interface ExpedienteRepository extends JpaRepository<Expediente, UUID> {
        ...
        @Query(value = "SELECT TO_CHAR(created_at, 'Month'), COUNT(*) FROM expedientes WHERE etapa_procesal = :etapa GROUP BY TO_CHAR(created_at, 'Month')", nativeQuery = true)
        List<Object[]> contarExpedientesPorEtapa(@Param("etapa") String etapa);

        @Query(value = "SELECT TO_CHAR(created_at, 'Month'), COUNT(*) FROM expedientes WHERE etapa_procesal = :etapa AND gerencia_id = :gerenciaId GROUP BY TO_CHAR(created_at, 'Month')", nativeQuery = true)
        List<Object[]> contarExpedientesPorEtapaYGerencia(@Param("etapa") String etapa,
                        @Param("gerenciaId") Long gerenciaId);

        @Query(value = "SELECT TO_CHAR(created_at, 'Month'), COUNT(*) FROM expedientes WHERE etapa_procesal = :etapa AND abogado_responsable_id = :abogadoId GROUP BY TO_CHAR(created_at, 'Month')", nativeQuery = true)
        List<Object[]> contarExpedientesPorEtapaYAbogado(@Param("etapa") String etapa,
                        @Param("abogadoId") Integer abogadoId);

        // ==========================================
        // 👤 MÉTODOS PARA DASHBOARD PERSONAL (ROD-12)
        // ==========================================
        long countByAbogadoResponsableId(Integer id);

        long countByEtapaProcesalAndAbogadoResponsableId(EtapaProcesal etapa, Integer id);

        @Query("SELECT e.etapaProcesal, COUNT(e) FROM Expediente e WHERE e.abogadoResponsable.id = :abogadoId GROUP BY e.etapaProcesal")
        List<Object[]> contarExpedientesPorEstatusYAbogado(@Param("abogadoId") Integer abogadoId);
}