...
public class AudienciaNotificacionScheduler {
    ...
    //@Scheduled(fixedRate = 10000) // <--- NUEVA (PARA PROBAR) comenta el otro y descomenta este para pruebas 
        ...
        for (Audiencia a : audiencias) {
            ...
            if (destinatario != null) {
                crearNotificacion(
                    destinatario,
                    "⚠️ ¡Audiencia MAÑANA!",
                    "Expediente: " + a.getExpediente().getNumero() + ". Prepárate, es mañana a las " + a.getHoraAudiencia(),
                    "AUDIENCIA",
                    a.getId().toString(),
                    Prioridad.ALTA 
                );
    ...
    public void notificarAudienciasHoy() {
        ...
        List<Audiencia> audiencias = audienciaRepository.findByFechaAudienciaAndHoraAudienciaBetweenAndEstatusAudienciaNot(
            hoy, ahora, enDosHoras, "CONCLUIDA"
        );
        ...
        for (Audiencia a : audiencias) {
            ...
            if (destinatario != null) {
                crearNotificacion(
                    destinatario,
                    "⏰ ¡AUDIENCIA INMINENTE!",
                    "Es hoy a las " + a.getHoraAudiencia() + ". Sala/Link: " + (a.getEsVirtual() ? "Virtual" : a.getSalaLugar()),
                    "AUDIENCIA",
                    a.getId().toString(),
                    Prioridad.ALTA
                );
    ...
    private Usuario obtenerDestinatario(Audiencia a) {
        if (a.getAbogadoComparece() != null) return a.getAbogadoComparece();
        if (a.getExpediente().getAbogadoResponsable() != null) return a.getExpediente().getAbogadoResponsable();
        return null;
    }

    private void crearNotificacion(Usuario user, String titulo, String mensaje, String entidadTipo, String entidadId, Prioridad prioridad) {
