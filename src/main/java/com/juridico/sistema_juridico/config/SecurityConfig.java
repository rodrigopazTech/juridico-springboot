    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http
                ...
                .authorizeHttpRequests(auth -> auth
                        ...
                        .requestMatchers("/usuarios", "/usuarios/**", "/agenda", "/agenda/**")
                        .hasAnyAuthority("DIRECCION", "SUBDIRECCION")

                        // 3. NIVEL MEDIO: Dashboard (Dirección, Subdirección, Gerentes, Jefes y
                        // Abogados)
                        .requestMatchers("/dashboard", "/dashboard/**")
                        .hasAnyAuthority("DIRECCION", "SUBDIRECCION", "GERENTE", "JEFE_DEPTO", "ABOGADO")

                        // 4. NIVEL OPERATIVO: Expedientes, Términos, Audiencias (TODOS los
                        // autenticados)
                        .requestMatchers("/expedientes", "/expedientes/**", "/terminos", "/terminos/**", "/audiencias",
                                "/audiencias/**")
                        .authenticated()
